...
gnomadData = fread("../source_data/wang2020_eds_score.tsv.gz")
geneBackground = fread("../source_data/gene_enhancer_frequency.tsv.gz")
...
ggplot(mergedData, aes(x=oe_lof_upper, y=observed) ) + 
  geom_bin2d( bins = 80) +
  annotate("text", x = Inf, y = Inf, label = text, hjust = 1.05, vjust = 1.5, size = 5.5, fontface = "bold"  ) +
  # geom_point( fill="#69b3a2", size = 0.5, alpha = 0.5) +
  geom_smooth(method = "lm") +
  labs(x="EDS score", y = "Observed combinations")+
...
## Significant enh-enh
enhEnhData = fread("../source_data/significant_enh_enh.tsv.gz", header = T, sep = "\t")
...
observed = fread("../source_data/gene_enhancer_frequency.tsv.gz", header = T, sep = "\t")
...
ggplot(mergedData, aes(x=oe_lof_upper, y=total_enh) ) + 
  ...
  geom_smooth(method = "lm") +
  scale_fill_gradient(trans = "log10", high = "#3f007d", low = "#dadaeb") +
  labs(x="EDS score", y = "# nearby enhancers")+
  theme_linedraw() + 
