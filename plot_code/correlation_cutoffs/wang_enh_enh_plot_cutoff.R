# Script to plot essentiality related plots

library(data.table)
library(ggplot2)

wangData = fread("../source_data/wang2020_eds_score.tsv")
geneBackground = fread("/home/dribeiro/EnhEnhPaper/data/gene_enhancer_frequency.tsv")
geneBackground = geneBackground[sign_enh > 0]
wangData = wangData[gene_id %in% geneBackground$gene][!is.na(oe_lof_upper)]

## Significant enh-enh
enhEnhData = fread("../../source_data/correlation_cutoffs/significant_enh_enh_cutoff0.1.tsv", header = T, sep = "\t")
# enhEnhData = fread("../../source_data/correlation_cutoffs/significant_enh_enh_bonferroni0.05.tsv", header = T, sep = "\t")
...
mergedData = merge(observed,wangData, by.x="gene",by.y = "gene_id")
...
ggplot(mergedData, aes(x=oe_lof_upper, y=observed) ) + 
  ...
  labs(x="EDS score", y = "Significant enhancer pairs")+
