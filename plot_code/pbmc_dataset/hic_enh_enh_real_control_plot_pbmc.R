...
##########Load/process data
data = fread("../../source_data/pbmc_dataset/pbmc_hic_contacts_significant_positive.bed.gz", header = T, sep = "\t")
...
ggplot(data[real == 1], aes(x = corr, y = res) ) +
  ...
  annotate("text", x = Inf, y = Inf, label = text, hjust = 1.05, vjust = 1.5, size = 9, fontface = "bold"  ) +
  xlab("Enhancer-enhancer correlation") +
  ylab("Hi-C contact (log distance-scaled)") +
  theme_linedraw() +
  theme(text = element_text(size = 30), # big screen
